...
\title{\textbf{Specification-Driven Verification:\\
...
\begin{abstract}
We present \textsf{specsaver}, a toolchain in which behavioural
specification is the single source of semantic truth.  A system's
behaviour is described once --- in a contract language whose clauses
are ordinary Python lambdas --- and every downstream artefact is
derived from it: executable pre/post-condition and frame checks on
Gherkin scenario data, structured telemetry assertions, and finally
machine-checked proof obligations in Rocq (Coq) over a weakest-precondition
calculus for a small imperative language (SnakeletExn) with a FunSpecS
specification kernel.  Domain authors declare a \emph{domain} once
(DDL, row types, contracts, Gherkin features); the framework supplies
scenario runners, semantic frame checking, differential-fidelity stubs
for library \emph{theories} (SQL, logging, OpenTelemetry), and an
obligation generator.  We describe the architecture, the contract
language, the theory mechanism by which third-party libraries are
adorned with semantics, and worked examples against SQLite through
SQLAlchemy.  The lowering pipeline produces a per-obligation scoreboard of
\textsc{proved} and \textsc{unknown} entries; a third status,
\textsc{disproved} (a runtime counterexample found by the scenario
runner), is a natural extension of the existing runtime checking.
\end{abstract}
...
Software testing practice stratifies along a familiar axis.  At one
end, unit tests assert concrete facts about concrete executions.  At
the other, formal verification proves universal properties over all
executions.  Between them lies a chasm: behavioural specifications
(BDD scenarios, design-by-contract assertions) that are expressive but
not machine-checked, and property-based tests that are general but
disconnected from any formal basis.

\textsf{specsaver} is an attempt to close that chasm by making one
artifact --- the \emph{contract} --- serve all of them at once.  The
core thesis:

\begin{principle}[Single source of semantic truth]
A contract, written once in the contract language, is simultaneously:
(1) an executable specification checked against concrete scenario data
drawn from Gherkin tables; (2) a generator of test artefacts
(witnesses, admissibility checks, frame soundness, telemetry
...
\end{tikzpicture}
\caption{The specification-driven verification pipeline.  A feature's
Examples rows are the test data; the contract is the executable and
provable specification; the lowering emits Coq obligations proven
against a hand-verified kernel.}
\label{fig:pipeline}
\end{figure}
...
Database-backed services fail in characteristic ways: partial writes
across transaction boundaries, leaked rows outside the declared frame,
telemetry that does not reflect committed state, and exception paths
that mutate what they should not.  These faults are invisible to unit
tests of the happy path and costly to prevent by review alone.

The standard tool for such reasoning is Design-by-Contract, but
classical DbC checks contracts at runtime \emph{only}, leaving
universal claims unproven; while interactive theorem proving at the
source-code level is powerful but rarely connected to the tests
developers already write.  We seek a middle course: contracts that are
checked on concrete data (so the specification is grounded in
executable reality) and that lower to a formal model (so the
specification is proved to be internally sound).
...
\textsf{specsaver} demonstrates that a single declarative contract can
drive the full spectrum from behavioural feature, through
example-grounded testing, to machine-checked proof --- without
modifying production code and without duplicating the specification.
...
production-stolen invitation flow --- show that the idiom scales to
multi-table, multi-exit, insert-bearing systems.  The
\textsc{proved}/\textsc{unknown}/\textsc{disproved} scoreboard
completes the feedback cycle: every row of every feature is a test;
every contract is a proof obligation; and every runtime failure of
a well-formed contract is a counterexample ready to be surfaced.
