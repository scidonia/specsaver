...
The lowering pipeline --- Python lambdas $\to$ AST introspection
$\to$ shape extraction $\to$ Rocq obligation generation over a
hand-verified WP calculus --- is a novel engineering combination.
Why3 extracts to Coq, but from a dedicated specification language
...
parallel proof generation (\cref{sec:lowering}) is likewise novel.
...
specifications (FunSpecS pre/post over a heap model); this is
implemented and the generated obligations are machine-proved.
\textbf{Implementation lowering} would translate the Python service
function --- the SQLAlchemy code inside \texttt{reserve()} --- into a
SnakeletExn \emph{program}, which could then be proved to refine the
contract specification.  This second direction is future work.
Currently the connection between implementation and contract is
established empirically by the scenario runner
(\cref{sec:architecture}), which exercises the Python code against
concrete Gherkin rows; it is a stated trust boundary, not a hidden
one.
...
Store obligations proved & 16/16 (automated) \\
...
and OpenTelemetry with validation suites, and the lowering pipeline
(introspect, emit, score).  All 23 store obligations are machine-proved
against the Rocq kernel.

Future work includes trace-cell obligations in the lowering, incremental
re-verification keyed on content hashes, loop and sequence support,
and surfacing disproved counterexamples from the runner.
