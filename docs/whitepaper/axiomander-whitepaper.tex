...
The lowering pipeline lowers a Python \texttt{Contract} into the
SnakeletExn language: the pre/post predicates become SnakeletExn
expressions over a two-cell heap (a store dict and a trace dict),
the invariants become SnakeletExn predicates, and the operation
itself becomes a stateful specification in the FunSpecS calculus.
The resulting Coq obligation file is scored against the
hand-verified SnakeletExn kernel.  The pipeline has three stages:
